...
ESP8266WebServer::ESP8266WebServer(std::shared_ptr<DispatcherServlet> dispatcherServlet, IPAddress addr, int port)
: _server(addr, port)
...
ESP8266WebServer::ESP8266WebServer(std::shared_ptr<DispatcherServlet> dispatcherServlet, int port)
: _server(port)
, _currentMethod(HTTP_ANY)
, _currentHandler(0)
, _firstHandler(0)
...
void printServletRequest(HttpServletRequest& req){
	Serial.println("The Request parameters:");
	Serial.println("url: " + req.url);
	Serial.print("Method: ");
	Serial.println((req.httPMethod == HTTP_GET ? "GET" : "POST"));
	Serial.println("Body: " + req.requestBody);
	Serial.println("url tokens:");
	for (int i = 0; i < req.urlTokens.size(); ++i) {
		Serial.print("	-token: ");
