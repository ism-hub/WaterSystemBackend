...
ESP8266WebServer::ESP8266WebServer(DispatcherServlet& dispatcherServlet, IPAddress addr, int port)
: _server(addr, port)
...
ESP8266WebServer::ESP8266WebServer(DispatcherServlet& dispatcherServlet, int port)
: _server(port)
, _currentMethod(HTTP_ANY)
, _currentHandler(0)
, _firstHandler(0)
...
ESP8266WebServer::~ESP8266WebServer() {
  ...
  _dispatcherServlet.~DispatcherServlet();//### check
  close();
}
...
HttpServletRequest* ESP8266WebServer::_createHttpServletRequest(){
    ...
    return new HttpServletRequest(requestBody, _currentMethod, _currentUri);
}
...
void ESP8266WebServer::handleClient() {
  ...
  if (_currentStatus == HC_WAIT_READ) {
    ...
    Serial.println ( "url: " + httpServletRequest->url );
    Serial.print ( "Method: ");
    		Serial.println ( (httpServletRequest->httPMethod == HTTP_GET ? "GET" : "POST"));
    Serial.println ( "Body: " + httpServletRequest->requestBody );
    Serial.println ( "url tokens:" );
    for (int i = 0;  i < httpServletRequest->urlTokens.size(); ++i) {
    	Serial.print ( "	-token: " );
    	Serial.println ( httpServletRequest->urlTokens[i] );
	}


    Serial.printf("settings heap size: %u\n", ESP.getFreeHeap());
    Serial.println ( "Dispatching the request   Calling _dispatcherServlet.dispatch(*httpServletRequest)" );
    HttpServletResponse* httpServletResponse = _dispatcherServlet.dispatch(*httpServletRequest);
    ...
    //here we need cleanup the requests
