...
//		Serial.println(__PRETTY_FUNCTION__);
	//}
	JsonLoadArchive(DynamicJsonBuffer& jsonBuffer, std::vector<DAL::JsonContex>& contexList) : jsonBuffer(jsonBuffer), contexList(contexList) {
	//	Serial.println("--------------------------- JsonLoadArchive CTOR");
	//	Serial.printf("settings heap size: %u\n", ESP.getFreeHeap());
	}
	~JsonLoadArchive() {
	//	Serial.println("~~~~~~~~~~~~~~~###############~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~ JsonLoadArchive DTOR");
//		Serial.println(__PRETTY_FUNCTION__);
	}
	...
	//	Serial.println("%%%%%%%%%%%%%%%%%%%%%%%%%%JsonLoadArchive befor _mappingFile.Json2Model");
	...
	//	Serial.println("%%%%%%%%%%%%%%%%%%%%%%%JsonLoadArchive after _mappingFile.Json2Model");
		//calling to start to serialize the model properties (adding the keys our user didn't add)
		model.load(mappingFile, *this);
	...
	template <class MappingFile, class T, class ... Other> inline
	void loadProperties(MappingFile& mappingFile, T&& head, Other&& ... tail )
	{
		loadProperty(head, mappingFile);//I think head is lvalue now (usually T&& is rvalue when we get NameValuePair)
		loadProperties(mappingFile, tail ... );
	}
	template <class MappingFile>
	void loadProperties(MappingFile& ){}

	template<class T, class MappingFile>
	void loadProperty(NameValuePair<T>& head, MappingFile& mappingFile){
		...
		loadProperty(head.value, mappingFile);//continue
	}

protected:

	//int/string/double/bool properties; we not nameValuePair so we are inside an array
	template<class T, typename std::enable_if<std::is_arithmetic<typename std::remove_reference<T>::type>::value || std::is_same<typename std::remove_reference<T>::type, String>::value   , sfinae>::type = {}, class MappingFile>
	void loadProperty(T& t, MappingFile& ) {
		auto& contex = contexList.back();
	...
	template<class T, typename std::enable_if<has_load_method<T>::value ,  sfinae>::type = {}, typename MappingFile>
	void loadProperty(T& t, MappingFile& mappingFile ) {
		...
		t.load(mappingFile, *this);//basically calling loadProperties function with the nameValuePairs of the properties we wants from the json and where to put them (refernce)
	...
	template<class T, typename A, typename MappingFile>
	void loadProperty(std::vector<T, A>&  vec, MappingFile& mappingFile) {
		...
		for(int i = 0; i < jArr.size(); i++){
			...
			if(pt != NULL){//TODO: add something in the lines of contexList.back()[i]
				loadProperty(*pt, mappingFile);
			}else{
				vec.push_back(T());
				loadProperty(vec.back(), mappingFile);
			}
		}
		contexList.pop_back();
	}
	...
	template <typename T, typename MappingFile>
	void loadProperty(std::shared_ptr<T>& sharedPtr, MappingFile& mappingFile) {
		auto& contex = contexList.back();
		...
		loadProperty(*sharedPtr, mappingFile);
	}
};
