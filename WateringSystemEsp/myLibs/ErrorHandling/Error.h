...
public:
	...
	Error(const Error<ErrorValueType>& err2) : errorCode(err2.errorCode), details(err2.details)
	{
		if(err2.errorCode == ErrorCode::NO_ERROR){
			Serial.println(F("__ERROR: wrong use of Error() copy constructor class"));
	...
	static String errorCodeToString(const ErrorCode errorCode){
		switch(errorCode){
		case ErrorCode::NO_ERROR: return F("NO_ERROR");
		case ErrorCode::ERR_SPI_CS_INVALID: return F("ERR_SPI_CS_INVALID");
		case ErrorCode::ERR_SPI_CS_TAKEN: return F("ERR_SPI_CS_TAKEN");
		case ErrorCode::ERR_SPI_CS_ALREADYFREE: return F("ERR_SPI_CS_ALREADYFREE");
		case ErrorCode::ERR_SPI_FAILED_SENDDATA: return F("ERR_SPI_FAILED_SENDDATA");
		default:
			return F("{MISSING ERROR TO STRING CONVERSION IN Error::errorCodeToString}");
		}
	}
