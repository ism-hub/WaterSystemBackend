...
public:
	PumpController(std::shared_ptr<garden::GardenUnitOfWork> unitOfWork, std::shared_ptr<hrdwrctrl::SwitchArray> switchHub, int pumpSwitchNumber) : _unitOfWork(unitOfWork), pumpSwitchNumber(pumpSwitchNumber), _switchHub(switchHub) {
	...
	//	Serial.println("@@@@@@@@@@ Changing pump status @@@@@@@@@@");
		...
		if(retVal != 1){
			Serial.println(F("__Some error accured in _switchHub->setSwitchStatus inside pump controller."));
