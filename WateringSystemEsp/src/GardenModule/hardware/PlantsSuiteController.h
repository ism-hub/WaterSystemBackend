...
class PlantsSuiteController {
	...
	public:
		std::weak_ptr<garden::Plant> _plant;

		PlantSuiteConroller(std::weak_ptr<garden::Plant> plant, std::shared_ptr<sched::SchedulerService> scheduleService) : _scheduleService(scheduleService), _plant(plant) {
			addHooks();
		...
		void addProgramTimepatternHooks(){
			...
			if(_plant.lock()->_program.get() != nullptr) {
				auto lambdaB4ProgramTimePatternChange = [=](garden::TimePattern timePattern){return this->beforeProgramTimePatternChange(timePattern);};
				programTimePatternB4HookId = _plant.lock()->_program.get()->timePattern.before_change().connect(lambdaB4ProgramTimePatternChange);
				auto lambdaOnProgramTimePatternChange = [=](garden::TimePattern timePattern){return this->onProgramTimePatternChange(timePattern);};
		...
		//	Serial.print("plantSuite addHooks");Serial.println(_plant.lock()->id);
			//program hooks
			auto lambdaBeforeProgramChange = [=](std::shared_ptr<garden::SimpleProgram> program){return this->beforProgramChange(program);};
			plantProgramB4HookId = _plant.lock()->_program.before_change().connect(lambdaBeforeProgramChange);
			auto lambdaOnProgramChange = [=](std::shared_ptr<garden::SimpleProgram> program){return this->onProgramChange(program);};
			plantProgramHookId = _plant.lock()->_program.on_change().connect(lambdaOnProgramChange);

			addProgramTimepatternHooks();

			//sprinkler hook
			auto lambdaB4PlantSprinklerChange = [=](std::shared_ptr<garden::Sprinkler> sprinkler){return this->beforePlantSprinklerChange(sprinkler);};
			plantSprinklerHookId = _plant.lock()->_sprinkler.before_change().connect(lambdaB4PlantSprinklerChange);
			auto lambdaOnPlantSprinklerChange = [=](std::shared_ptr<garden::Sprinkler> sprinkler){return this->onPlantSprinklerChange(sprinkler);};
		...
		//		Serial.println("plantSuite executePlantProgram don't have sprinkler or program so we won't execute");
			...
			for(auto& day : days){
				for(auto& hour : day.hours){
					...
					garden::Hour hourPlusFewSec = hour;
		...
		void beforProgramChange(std::shared_ptr<garden::SimpleProgram> newProgram) {
		...
		void onProgramChange(std::shared_ptr<garden::SimpleProgram> newProg) {
			addProgramTimepatternHooks();
			executePlantProgram();//runs the new one
		}

		void beforeProgramTimePatternChange(garden::TimePattern newTimePattern) {// make sure that the plant still have that program
		...
		void onProgramTimePatternChange(garden::TimePattern newTimePattern) {// make sure that the plant still have that program
		//	Serial.print("plantSuite onProgramTimePatternChange, plant id - ");Serial.print(_plant.lock()->id);
		//	Serial.print(" program id - ");Serial.println(_plant.lock()->_program.get()->id);
			executePlantProgram();
		}

		void beforePlantSprinklerChange(std::shared_ptr<garden::Sprinkler> newSprinkler) {
		...
		void onPlantSprinklerChange(std::shared_ptr<garden::Sprinkler> newSprinkler) {
	...
	//		Serial.print("plantSuite startWaterPlant ");Serial.println(_plant.lock()->id);
			_plant.lock()->_sprinkler.get()->status = garden::Sprinkler::Status::On;
			_plant.lock()->garden->_pump->status = garden::Pump::On;
		}
	...
	//		Serial.print("plantSuite stopWaterPlant ");Serial.println(_plant.lock()->id);
			if(_plant.lock()->garden->_pump != nullptr)
				if(isAllSprinklersWillBeClose(_plant.lock()->_sprinkler.get()->id))// if all sprinklers are close and the pump is working the water dont have a  way put, we need to close the pump first and than close the last sprinkler
					_plant.lock()->garden->_pump->status = garden::Pump::Off;
			if (_plant.lock()->_sprinkler.get() != nullptr) {
				_plant.lock()->_sprinkler.get()->status = garden::Sprinkler::Status::Off;
			}
		}
	...
	//		Serial.println("inside isAllSprinklersWillBeClose");
			for(auto& sprinkler : _plant.lock()->garden->_sprinklers.getInnerVector()){
				if((sprinkler.get()->id != id) &&
					(sprinkler.get()->status == garden::Sprinkler::Status::On)){
...
public:
	PlantsSuiteController(std::shared_ptr<sched::SchedulerService> scheduleService, std::shared_ptr<garden::GardenUnitOfWork> unitOfWork)
		: _scheduleService(scheduleService), _unitOfWork(unitOfWork)
	{
		...
		auto lambdaOnPlantsChange = [=](Model::Property<std::shared_ptr<garden::Plant>> plant, Model::Change change){return this->onPlantsChange(plant, change);};
		hookId = _unitOfWork->Garden().getById(-1)->_plants.on_change().connect(lambdaOnPlantsChange);

	}

	~PlantsSuiteController() {
...
protected:
	...
	void onPlantsChange(Model::Property<std::shared_ptr<garden::Plant>> plant, Model::Change change) {
	...
	void onNewPlant(Model::Property<std::shared_ptr<garden::Plant>> plant) {
	...
	void onPlantDelete(Model::Property<std::shared_ptr<garden::Plant>> plant) {
