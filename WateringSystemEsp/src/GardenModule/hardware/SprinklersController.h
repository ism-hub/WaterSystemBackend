...
#include <HardwareLib/chips/SwitchArray.h>
...
class SprinklersController {
	...
	public:
		std::weak_ptr<garden::Sprinkler> _sprinkler = {};
		int hookOnStatusChangeId;
		std::shared_ptr<hrdwrctrl::SwitchArray> _switchHub = nullptr;

		SprinklerController(std::weak_ptr<garden::Sprinkler> sprinkler, std::shared_ptr<hrdwrctrl::SwitchArray> switchHub) : _sprinkler(sprinkler), _switchHub(switchHub) {
	...
	//			Serial.println("_sprinkler is expired");
		...
		//	Serial.println("@@@@@@@@@@ Changing sprinkler status @@@@@@@@@@");
			if(dryRun){
				Serial.println(F("__doing dryRun to SprinklersConroller onStatusChange."));
				dryRun = false;
				return;
			}
...
public:
	SprinklersController(std::shared_ptr<garden::GardenUnitOfWork> unitOfWork, std::shared_ptr<hrdwrctrl::SwitchArray> switchHub) : _unitOfWork(unitOfWork), _switchHub(switchHub) {
		...
		auto lambdaOnSprinklersChange = [=](std::shared_ptr<garden::Sprinkler> sprinkler, Model::Change change){return this->onSprinklersChange(sprinkler, change);};
		hookId = _unitOfWork->Garden().getById(-1)->_sprinklers.on_change().connect(lambdaOnSprinklersChange);
	}

	~SprinklersController() {
...
//		Serial.print("Number of controllers: ");Serial.println((int)(_controllers.size()));
	...
	void onSprinklersChange(std::shared_ptr<garden::Sprinkler> sprinkler, Model::Change change) {
//		Serial.println(" called onSprinklersChange");
	...
	void onNewSprinkler(std::shared_ptr<garden::Sprinkler> sprinkler){
//		Serial.println(" onSprinklersChange - Added");
	...
	void onSprinklerDelete(std::shared_ptr<garden::Sprinkler> sprinkler){
	...
	//		Serial.println("Actually deleted a sprinkler");
			...
			Serial.println(F("ERROR? didn't deleted a sprinkler sprinkler 404 in the list"));
	}
};
